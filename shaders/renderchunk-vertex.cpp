...
// На прозрачных штуках
	...
	if(color.g > color.b){ // Если они зеленые
		POS3 l = POSITION.xyz;
		gl_Position.s += sin(TIME * 0.783 + 3.0*l.x+l.y+l.z) * 0.05; // Применить искажение
	}
#endif
///// apply fog
...
//		vec3 sinedata = xyz * 20.0 + TIME * 1.5;  //Скорость: 6.0(или в конфиге)
		gl_Position.x += sin(POSITION.z * 20.0 + TIME * 1.5) * cos(POSITION.z * 20.0 + TIME * 1.5) * range; //Искажение по Z
		gl_Position.y += sin(POSITION.x * 20.0 + TIME * 1.5) * cos(POSITION.x * 20.0 + TIME * 1.5) * range; //Искажение по X
		gl_Position.z += sin(POSITION.y * 20.0 + TIME * 1.5) * cos(POSITION.y * 20.0 + TIME * 1.5) * range; //Искажение по Y
...
///// water magic
		...
		// Волны на воде
		gl_Position.y += sin(TIME * 2.0 + 3.0*POSITION.x+POSITION.y+POSITION.z) * 0.05;
